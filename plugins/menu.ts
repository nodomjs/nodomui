...
class UIMenu implements nodom.IDefineElement{
    ...
    init(el:HTMLElement){
        ...
        //显示字段名
        let showName:string = menuDom.getProp('showname');
        //激活字段名
        this.activeName = '$nui_menu_' + nodom.Util.genId();
        this.menuStyleName = '$nui_menu_' + nodom.Util.genId();
        //最大级数，默认3
        let maxLevels:number = menuDom.hasProp('maxlevels')?parseInt(menuDom.getProp('maxlevels')):3;
        //宽度
        let menuWidth:string = menuDom.getProp('width') || '200px';
        //展开收拢事件
        let methodId = '$nodomGenMethod' + nodom.Util.genId();
        let closeOpenEvent:nodom.NodomEvent = new nodom.NodomEvent('mouseover', 
