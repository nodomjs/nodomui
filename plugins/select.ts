...
class UISelect implements nodom.IDefineElement{
    ...
    init(el:HTMLElement){
        ...
        let dataName:string = selectDom.props['field'];
        let showName:string = selectDom.props['show'];

        //显示框
        let content:nodom.Element = new nodom.Element('div');
        ...
        //点击展开或收拢
        icon.addEvent(new nodom.NodomEvent('click',
            (dom,model,module)=>{
        ...
        //下拉框
        let list:nodom.Element = new nodom.Element('div');
